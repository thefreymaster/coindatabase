angular.module('coindbApp').service('reusableDataService', ['httpService', 'bigScreenService', '$localStorage', '$mdBottomSheet', '$mdSidenav', '$state', 'accountHttpService', function (httpService, bigScreenService, $localStorage, $mdBottomSheet, $mdSidenav, $state, accountHttpService) {
    var service = this;
    ...
    //CHECK IF USERID EXISTS IN LOCAL STORAGE, IF NOT, SUBMIT TO API/ACCOUNT/NEW ELSE SUBMIT TO API/ACCOUNT/:ID

    if (service.$storage.accountID == undefined) {
        var accountResponse = service.accountHttpService.createNewAccount();
        accountResponse.then(function(results){
            console.log(results);
            service.$storage.accountID = results.data._id;
        })
        
    }
    console.log(service.$storage.accountID)
